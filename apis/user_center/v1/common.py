...
@router.get(
    ...
    description="枚举-列表",
    summary="枚举表",
    response_model=Resp[dict | tuple[tuple]],
)
async def enum_content(
    ...
    if format == enums.RespFormatEnum.list_:
        ...
        for k, v in enum_content.items():
            data[k] = [{"key": k, "label": v} for k, v in v.items()]
        return Resp[dict](data=data)  # type: ignore
    return Resp[dict](data=enum_content)  # type: ignore
